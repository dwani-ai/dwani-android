...
class DocsActivityOld : MessageActivity() {
            ...
            try {
                ...
                withContext(Dispatchers.IO) {
                    ...
                    if (isImage) {
                        ...
                        val prefs = PreferenceManager.getDefaultSharedPreferences(this@DocsActivityOld)
                        ...
                        withContext(Dispatchers.Main) {
                            Toast.makeText(this@DocsActivityOld, "Unsupported file type. Please upload an image or PDF.", Toast.LENGTH_LONG).show()
                        }
                    }
                }
            } catch (e: Exception) {
                ...
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@DocsActivityOld, "File processing failed: ${e.message}", Toast.LENGTH_LONG).show()
                }
            } finally {
                withContext(Dispatchers.Main) {
        ...
        lifecycleScope.launch {
            ApiUtils.performApiCall(
                ...
                apiCall = {
                    ...
                    RetrofitClient.apiService(this@DocsActivityOld).visualQuery(
                        filePart,
                        dataBody,
                        "Bearer $token",
                        cleanSessionKey
                ...
                onError = { e ->
                    ...
                    Toast.makeText(this@DocsActivityOld, "Visual query failed: ${e.message}", Toast.LENGTH_LONG).show()
                }
            )
        }
    }
    ...
    private fun getPdfSummaryResponse(file: File) {
        ...
        lifecycleScope.launch {
            try {
                ...
                Toast.makeText(this@DocsActivityOld, "PDF summarization failed: ${e.message}", Toast.LENGTH_LONG).show()
            } finally {
                progressBar.visibility = View.GONE
            }
        }
